...
class Program
{
    ...
    class Book
    {
        ...
        virtual public string view(bool show)
        {
            string result = $"Showing info about book : \n Title : {title} \n Author : {author.view(false)} \n releasedAt : {releasedAt.ToString()}";
            if (show) {
    ...
    class AdventureBook : Book { 
        ...
        public AdventureBook(string title, Person author, DateTime releasedAt, string description) : base(title, author,releasedAt)
        {
            this.description = description;
        }

        override public string view(bool show) {
    ...
    class DocumentaryBook : Book {
        ...
        private string refersToEra;

        public DocumentaryBook(string title, Person author, DateTime releasedAt, string refersToEra) : base(title, author, releasedAt)
        {
            this.refersToEra = refersToEra;
        }
        override public string view(bool show)
        {
            string result = base.view(false) + "\n description : " + refersToEra;
            if (show) {
                Console.WriteLine(result);
            }
            return result;
    ...
    public static void Main(String[] args)
    {
        ...
        AdventureBook book1 = new AdventureBook(title1, author1, releasedAt1, "opis123");
        DocumentaryBook book2 = new DocumentaryBook(title2, author2, releasedAt2, "średniowiecze");
        AdventureBook book3= new AdventureBook(title3, author3, releasedAt3, "opis456");
        ...
        Reader reader = new Reader("Andrzej", "Duda", 42, books1);
        Reader reviewer = new Reviewer("Jerzy", "Aksamit", 69, books2);

        List<Reader> persons = new List<Reader>();

        persons.Add(reader);
        persons.Add(reviewer);

        persons.ForEach((person) => { 
            Book [] books = person.getBooks();
            for (int i = 0; i < books.Length; i++) {
                books[i].view(true);
            }
        });
